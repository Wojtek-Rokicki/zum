...
spam_uni_15 <- read.csv("ngrams_vec/spam_freq_vec_uni_en_15.csv", header=FALSE, encoding = "UTF-8")
names(spam_uni_15)[length(names(spam_uni_15))]<-"class" 

ham_uni_15 <- read.csv("ngrams_vec/ham_freq_vec_uni_en_15.csv", header=FALSE, encoding = "UTF-8")
names(ham_uni_15)[length(names(ham_uni_15))]<-"class" 
...
data_ngrams_15 = rbind(spam_uni_15,ham_uni_15)

data_ngrams_15 %>%group_by(class) %>%summarise(n = n()) %>%mutate(Freq = n/sum(n))
